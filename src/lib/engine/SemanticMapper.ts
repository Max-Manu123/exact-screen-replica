...
function fuzzyMatch(text: string, target: string, threshold = 0.85): boolean {
  const words = text.split(/\s+/);
  for (const word of words) {
    if (similarity(word, target) >= threshold) return true;
  }
  return false;
}
  ...
  for (const key of Object.keys(hints) as T[]) {
    ...
    for (const hint of hints[key]) {
      ...
      if (text.includes(normalizedHint)) {
        score += hint.length > 4 ? 1.5 : 1;
      }
      // Fuzzy match for typos (only for longer words)
      else if (hint.length > 5 && fuzzyMatch(text, normalizedHint, 0.85)) {
        score += 1;
      }
    }
    if (score > bestScore) {
